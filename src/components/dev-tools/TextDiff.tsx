...
export default function TextDiff() {
  ...
  const compareTexts = () => {
    setDiffResult(diffLines(text1, text2))
  }

  const clearAll = () => {
    setText1(''); setText2(''); setDiffResult(null)
  }
  ...
  return (
    <Card>
      <div className="p-6 md:p-8 bg-gradient-to-r from-teal-50 to-emerald-50 border-b border-gray-100">
        <h2 className="text-2xl font-bold text-gray-900">📝 {t.devTools.textDiff}</h2>
        <p className="text-gray-600 mt-1">{t.devToolUI.textDiffDescription}</p>
      </div>
      <div className="p-6 md:p-8 space-y-6">
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-3">{t.devToolUI.originalText}</label>
            <textarea value={text1} onChange={(e) => setText1(e.target.value)}
              ...
              placeholder={t.devToolUI.inputTextDiff} />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-3">{t.devToolUI.compareText}</label>
            <textarea value={text2} onChange={(e) => setText2(e.target.value)}
              className="w-full h-48 p-4 border-2 border-gray-200 rounded-xl font-mono text-sm focus:ring-4 focus:ring-teal-100 focus:border-teal-500 transition-all resize-none"
              placeholder={t.devToolUI.inputCompareText} />
          </div>
        </div>
        ...
        {!diffResult && (
          <div className="p-4 bg-gradient-to-r from-gray-50 to-gray-100 border-2 border-gray-200 rounded-xl">
            <p className="text-gray-600 text-center font-medium">{t.devToolUI.enterTextAndCompare}</p>
          </div>
        )}
      </div>
    </Card>
