...
export default function WeightConverter() {
  ...
  return (
    <Card className="p-6 md:p-8">
      <h2 className="text-2xl font-bold text-gray-900 mb-6">{t.converters.weight}</h2>
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-3">
          <Input type="number" value={value} onChange={(e) => setValue(e.target.value)} placeholder={t.converterUI.inputPlaceholder} />
          <Select value={fromUnit} onChange={(e) => setFromUnit(e.target.value as typeof WEIGHT_UNITS[number])}>
            {WEIGHT_UNITS.map((unit) => (
              <option key={unit} value={unit}>{getWeightUnitName(unit, language)}</option>
            ))}
          </Select>
        </div>
        <div className="space-y-3">
          <Input type="number" value={formatNumber(result, 8)} readOnly placeholder={t.converterUI.resultPlaceholder} />
          <Select value={toUnit} onChange={(e) => setToUnit(e.target.value as typeof WEIGHT_UNITS[number])}>
            {WEIGHT_UNITS.map((unit) => (
              <option key={unit} value={unit}>{getWeightUnitName(unit, language)}</option>
            ))}
          </Select>
